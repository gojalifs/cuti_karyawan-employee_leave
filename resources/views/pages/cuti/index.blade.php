                ...
                <div class="col-12 col-sm-12 col-lg-12">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h4>Master Data Cuti</h4>
                            <a class="btn btn-action bg-blue mr-1" id="openModalLink" href="#">Tambah Data Cuti</a>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive table-invoice">
                                <table class="table table-striped" id="table-1">
                                    <tr>
                                        ...
                                        <th>Nama Cuti</th>
                                        <th>Jumlah Cuti (per tahun)</th>
                                    </tr>
                                    @foreach ($cuti as $i => $k)
                                        <tr>
                                            <td class="p-0 text-center">{{ $i + 1 }}</td>
                                            <td class="font-weight-600">{{ $k->nama_cuti }}</td>
                                            <td class="align-middle">{{ $k->jumlah }}</td>
                                            <td>
                                                <a class="btn btn-action bg-purple mr-1" id="edit-cuti-{{ $k->id }}"
                                                    data-id="{{ $k->id }}" data-name="{{ $k->nama_cuti }}"
                                                    data-jumlah="{{ $k->jumlah }}"
                                                    href="{{ route('cuti.edit', ['id' => $k->id]) }}">Edit</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </table>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="formModal">Form Tambah Data Cuti</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <form method="POST" action="{{ route('cuti.add') }}">
                            @csrf
